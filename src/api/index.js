...
// 发请求：axios 发请求返回结果是Promise 对象
export const reqCategoryList = () => requests.get(`/product/getBaseCategoryList`)

// 获取banner （Home 首页轮播接口）
...
// 当前这个接口，给服务器传递参数params，至少是个空对象
// export const reqGetSearchInfo = (params) => requests({ url: '/list', methods: 'post', data: params })    // 有问题未解决
export const reqGetSearchInfo = (params) => requests.post( '/list',params )

// 获取产品详情信息的接口   /api/item/{ skuId } 请求方式：get
// export const reqGoodsInfo = (skuId) => requests.get( `/list/${skuId}` )  // 这个又出错
export const reqGoodsInfo = (skuId)=>requests({url:`/item/${skuId}`,method:'get'});

// 将产品添加到购物车中或者更新某一个商品的个数 /api/cart/addToCart/{ skuId }/{ skuNum }    post
